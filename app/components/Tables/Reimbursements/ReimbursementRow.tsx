...
function StatusDot({ isApproved, rejectionNote }: { isApproved: boolean; rejectionNote?: string }) {
  const color = rejectionNote ? "bg-red-500" : isApproved ? "bg-green-500" : "bg-yellow-500";
  return (
    ...
    <div className="flex items-center justify-end gap-0.5">
      {showAdminActions && (
        <>
          ...
          <Button variant="ghost" size="icon" className="h-7 w-7 text-red-600 hover:text-red-700 hover:bg-red-50" onClick={onReject} disabled={hasRejection}>
            <X className="h-4 w-4" />
          </Button>
        </>
      )}
