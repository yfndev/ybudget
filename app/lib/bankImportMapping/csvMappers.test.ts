...
test("parses sparkasse date DD.MM.YY correctly", () => {
  const result = mapCSVRow({ Buchungstag: "15.03.24", Betrag: "0", Verwendungszweck: "test" }, "sparkasse");
...
test("parses sparkasse amount with German format", () => {
  const result = mapCSVRow({ Buchungstag: "01.01.24", Betrag: "1.234,56", Verwendungszweck: "test" }, "sparkasse");
  expect(result.amount).toBe(1234.56);
});

test("parses negative sparkasse amount", () => {
  const result = mapCSVRow({ Buchungstag: "01.01.24", Betrag: "-50,00", Verwendungszweck: "test" }, "sparkasse");
  expect(result.amount).toBe(-50);
});

test("parses volksbank date DD.MM.YYYY correctly", () => {
  const result = mapCSVRow({ Buchungstag: "15.03.2024", Betrag: "0", Verwendungszweck: "test" }, "volksbank");
  expect(new Date(result.date).getFullYear()).toBe(2024);
  expect(new Date(result.date).getMonth()).toBe(2);
  expect(new Date(result.date).getDate()).toBe(15);
});
...
test("parses moss date MM/DD/YYYY correctly", () => {
  const result = mapCSVRow({ "Payment Date": "03/15/2024", Amount: "100" }, "moss");
  expect(new Date(result.date).getFullYear()).toBe(2024);
  expect(new Date(result.date).getMonth()).toBe(2);
  expect(new Date(result.date).getDate()).toBe(15);
});
...
test("parses moss amount", () => {
  const result = mapCSVRow({ "Payment Date": "01/01/2024", Amount: "99.99" }, "moss");
  expect(result.amount).toBe(99.99);
});

test("returns 0 for empty amount", () => {
  const result = mapCSVRow({ Buchungstag: "01.01.24", Betrag: "", Verwendungszweck: "test" }, "sparkasse");
